...
int Storage::begin() {
  if(SD.begin(BUILTIN_SDCARD)){
    ...
    if(dataFile){
      ...
      dataFile.print("accelX"); dataFile.print(separator);
      dataFile.print("accelY"); dataFile.print(separator);
      dataFile.print("accelZ"); dataFile.print(separator);
...
bool Storage::saveSD(const Buffer& buffer){
  ...
  if (dataFile) {
    for (unsigned i = 0; i < buffer.size(); i++) {
      logEvent(buffer[i].event);

      dataFile.print(String(buffer[i].timestamp / 1000.0)); dataFile.print(separator);
